...
async function authBaseServer(): Promise<string> {
  const raw =
  ...
  if (resolved.startsWith("/")) {
    const h = (await headers()).get("host") || "localhost:3060";
...
async function serializeRequestCookies(): Promise<string> {
...
export async function getServerAuthSession(): Promise<ServerAuthSession> {
  const base = await authBaseServer();
  ...
  try {
    const res = await fetch(`${base}/auth/session`, {
      ...
      cache: "no-store",
      headers: {
        cookie: await serializeRequestCookies(),
